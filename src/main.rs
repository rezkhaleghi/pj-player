...
async fn run_app(
     ...
     *
     * More importantly, every blocking playback operation is kept away
     * from this thread so crossterm remains responsive.
     */
...
async fn handle_streaming(app: &mut AppUi, key: KeyEvent) -> Result<(), AppError> {
    match key.code {
        KeyCode::Esc => {
            /*
             * A seek is deliberately allowed to finish rather than being
             * detached/cancelled because spawn_blocking cannot safely abort
             * once it has started.
             */
            if app.seek_task.is_some() {
                return Ok(());
            }
